...
def settings():
    ...
    if len(secret) < 32 or len(signing) < 32:
        raise ValueError(
            "POLARIS_CLIENT_SECRET and TOKEN_SIGNING_KEY must have at least 32 characters"
        )
    client = os.environ.get("POLARIS_CLIENT_ID", "root")
    if any(c in client + secret for c in ",\r\n"):
        raise ValueError(
            "Polaris bootstrap credentials cannot contain commas or newlines"
        )
    name = os.environ.get("CATALOG_NAME", "warehouse")
    if not re.fullmatch(r"[a-zA-Z][a-zA-Z0-9_-]{0,62}", name):
        raise ValueError(
            "CATALOG_NAME must start with a letter and contain only letters, digits, _ or -"
        )
    ...
    if (
        urlsplit(endpoint).scheme not in ("https", "http")
    ...
    env.update(
        {
            ...
            "QUARKUS_DATASOURCE_JDBC_URL": f"jdbc:postgresql://{host}:{db.port or 5432}{db.path}"
            + (f"?{db.query}" if db.query else ""),
            ...
            "OTEL_RESOURCE_PROVIDERS_GCP_ENABLED": "false",
            "OTEL_RESOURCE_PROVIDERS_AWS_ENABLED": "false",
            "OTEL_RESOURCE_PROVIDERS_AZURE_ENABLED": "false",
            ...
            "POLARIS_AUTHENTICATION_TOKEN_BROKER_SYMMETRIC_KEY_SECRET": config[
                "signing"
            ],
    ...
    return {
        "catalog": {
            ...
            "storageConfigInfo": {
                ...
                "pathStyleAccess": config["path_style"],
                "stsUnavailable": True,
            },
        }
    }
